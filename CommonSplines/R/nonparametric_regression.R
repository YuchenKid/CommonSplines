...
np_reg<- function(x_train, y_train, x_test, func = 'bs',order=3,df = NULL, knots = NULL,lambda=0.001)
{
  if(func=="ncs"){
    fit <- ncs_train(x_train, y_train, df = df, knots = knots)
    y_pred <- ncs_predict(x_test, fit$betas,fit$knots)
  }
  else if(func=="bs"){
    fit <- bs_train(x_train, y_train, real_knots = knots)
    ...
    fit <- pbs_train(x_train, y_train,order=order, df = df, knots = knots)
...
generate_knots<-function(x_train,df,knots)
{
